...
class TorPlugin implements DuplexPlugin, EventListener
{
	...
	// Sulaman please access data here

	TorPlugin(Executor ioExecutor, Executor wakefulIoExecutor, NetworkManager networkManager, LocationUtils locationUtils, SocketFactory torSocketFactory,
			CircumventionProvider circumventionProvider, BatteryManager batteryManager, Backoff backoff, TorRendezvousCrypto torRendezvousCrypto, TorWrapper tor,
			PluginCallback callback, long maxLatency, int maxIdleTime)
	{
		...
		connectionStatusExecutor = new PoliteExecutor("TorPlugin", ioExecutor, 1);
	...
	public void start() throws PluginException
	{
		...
		try
		{
			try
			{
				System.out.println("THIS IS READ POINT");
				StorageUtils storageUtils = new StorageUtils();
				System.out.println("DATA = " + storageUtils.readFile("AR"));
			}
			catch (Exception e)
			{
				e.printStackTrace();
		...
		updateConnectionStatus(networkManager.getNetworkStatus(), batteryManager.isCharging());
	...
	private void enableBridges(List<BridgeType> bridgeTypes, String countryCode) throws IOException
	{
		...
		else
		{
			...
			{
				bridges.addAll(circumventionProvider.getBridges(bridgeType, countryCode));
			}
	...
	public DuplexTransportConnection createConnection(TransportProperties p)
	{
		...
		catch (IOException e)
		{
			if (LOG.isLoggable(INFO))
			{
				LOG.info("Could not connect to v3 " + scrubOnion(onion3) + ": " + e);
			}
	...
	@Override
	public DuplexTransportConnection createKeyAgreementConnection(byte[] commitment, BdfList descriptor)
	{
		throw new UnsupportedOperationException();
	}
	...
	@Override
	public RendezvousEndpoint createRendezvousEndpoint(KeyMaterialSource k, boolean alice, ConnectionHandler incoming)
	{
		...
		try
		{
			...
			ioExecutor.execute(() -> {
				try
				{
					...
					{
						Socket s = ss.accept();
						incoming.handleConnection(new TorTransportConnection(this, s));
					}
				}
				catch (IOException e)
				{
	...
	public void eventOccurred(Event e)
	{
		...
		{
			updateConnectionStatus(((NetworkStatusEvent) e).getStatus(), batteryManager.isCharging());
		}
		else if (e instanceof BatteryEvent)
		{
			updateConnectionStatus(networkManager.getNetworkStatus(), ((BatteryEvent) e).isCharging());
		}
	}

	private void updateConnectionStatus(NetworkStatus status, boolean charging)
	{
		connectionStatusExecutor.execute(() -> {
			...
			boolean bridgesByDefault = circumventionProvider.shouldUseBridges(country);
			boolean enabledByUser = settings.getBoolean(PREF_PLUGIN_ENABLE, DEFAULT_PREF_PLUGIN_ENABLE);
			int network = settings.getInt(PREF_TOR_NETWORK, DEFAULT_PREF_TOR_NETWORK);
			boolean useMobile = settings.getBoolean(PREF_TOR_MOBILE, DEFAULT_PREF_TOR_MOBILE);
			boolean onlyWhenCharging = settings.getBoolean(PREF_TOR_ONLY_WHEN_CHARGING, DEFAULT_PREF_TOR_ONLY_WHEN_CHARGING);
			boolean automatic = network == PREF_TOR_NETWORK_AUTOMATIC;

			if (LOG.isLoggable(INFO))
			{
				LOG.info("Online: " + online + ", wifi: " + wifi + ", IPv6 only: " + ipv6Only);
				if (country.isEmpty()) LOG.info("Country code unknown");
				else
					LOG.info("Country code: " + country);
			...
			else
			{
				...
				else
				{
					...
					enableNetwork = true;
					if (network == PREF_TOR_NETWORK_WITH_BRIDGES || (automatic && bridgesByDefault))
					{
						if (ipv6Only)
						{
						...
						{
							bridgeTypes = circumventionProvider.getSuitableBridgeTypes(country);
						}
						if (LOG.isLoggable(INFO))
						{
	...
	private class PluginState
	{
		...
		private synchronized State getState(TorState torState)
		{
			...
			if (torState == TorState.NOT_STARTED || torState == TorState.STARTING || torState == TorState.STARTED || torState == TorState.STOPPING || torState == TorState.STOPPED
					|| !settingsChecked)
			{
